...
async function requireAdmin(){const viewer=await getApiViewer();return viewer.user&&viewer.profile?.role==="admin"?viewer:null;}

export async function PATCH(request:NextRequest){
  const viewer=await requireAdmin();if(!viewer)return NextResponse.json({error:"Acesso negado."},{status:403});
  ...
  const admin=createAdminSupabaseClient();const {data:found,error:readError}=await admin.from("user_books").select("*").eq("id",id).single();
  if(readError||!found)return NextResponse.json({error:readError?.message||"Envio não encontrado."},{status:404});let item=found;

  if(status==="catalog"){
    const manualPdfId=String(body.readingPdfDriveFileId||"").trim();const manualPdfName=String(body.readingPdfFileName||"").trim();
    if(manualPdfId||manualPdfName){
      ...
      const now=new Date().toISOString();const {data:withPdf,error:pdfError}=await admin.from("user_books").update({reading_pdf_drive_file_id:manualPdfId,reading_pdf_file_name:manualPdfName,reading_pdf_generated_at:now,updated_at:now}).eq("id",id).select("*").single();
      if(pdfError||!withPdf)return NextResponse.json({error:pdfError?.message||"Não foi possível vincular o PDF."},{status:400});item=withPdf;
    }
  ...
  }else await admin.from("books").delete().eq("drive_file_id",item.drive_file_id);
...
export async function DELETE(request:NextRequest){
  const viewer=await requireAdmin();if(!viewer)return NextResponse.json({error:"Acesso negado."},{status:403});const id=request.nextUrl.searchParams.get("id");if(!id)return NextResponse.json({error:"ID obrigatório."},{status:400});
  const admin=createAdminSupabaseClient();const {data:item,error}=await admin.from("user_books").select("drive_file_id,reading_pdf_drive_file_id,kindle_drive_file_id").eq("id",id).maybeSingle();if(error)return NextResponse.json({error:error.message},{status:400});if(!item)return NextResponse.json({ok:true});
  const ids=[item.drive_file_id,item.reading_pdf_drive_file_id,item.kindle_drive_file_id].filter((v,i,a):v is string=>Boolean(v)&&a.indexOf(v)===i);for(const fileId of ids){try{await deleteDriveFile(fileId);}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"Falha ao excluir arquivo."},{status:502});}}
  await admin.from("books").delete().eq("drive_file_id",item.drive_file_id);const {error:deleteError}=await admin.from("user_books").delete().eq("id",id);return deleteError?NextResponse.json({error:deleteError.message},{status:400}):NextResponse.json({ok:true});
}
