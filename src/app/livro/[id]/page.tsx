...
export default async function BookPage({params}:{params:Promise<{id:string}>}){
  ...
  const {data:book}=await supabase.from("books").select("*,categories(name)").eq("id",id).eq("published",true).maybeSingle();
  if(!book)notFound();
  ...
  return <AppShell><main className="shell-width detail-page"><Link className="back-link" href="/biblioteca">← Voltar ao acervo</Link><section className="detail">
    ...
    <div className="detail-copy"><span className="eyebrow">ACERVO KINDLE BOOK</span><h1>{b.title}</h1><h2>{b.author}</h2><div className="detail-stats">{b.year&&<div><small>ANO</small><strong>{b.year}</strong></div>}{b.pages&&<div><small>PÁGINAS</small><strong>{b.pages}</strong></div>}{b.categories?.name&&<div><small>CATEGORIA</small><strong>{b.categories.name}</strong></div>}</div><div className="detail-actions"><Link className="btn" href={`/leitor/${b.id}`}>Ler agora</Link>{b.allow_download&&<a className="btn ghost" href={`/api/books/${b.id}/file?download=1`}>Baixar PDF</a>}</div><div className="synopsis-block"><span className="eyebrow">SINOPSE</span><div className="prose">{b.description||"Sinopse não informada."}</div></div></div>
